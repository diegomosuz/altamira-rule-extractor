...
def _program_with_decision(*, line_start: int = 10, literal: str = "0005") -> CanonicalProgram:
    if_stmt = make_stmt(
        statement_id="P1::A::0::IF", kind=StatementKind.IF, line_start=line_start,
        expression="CONDICION",
    )
    move = make_stmt(
        statement_id="P1::A::1::MOVE", target_data_items=["WS-COD-RETORNO"],
        variables_written=["WS-COD-RETORNO"], assigned_literal=literal,
...
def _v1_candidate(*, decision_id: str, outcome_code: str, line_start: int = 10) -> RuleCandidate:
    return RuleCandidate(
        ...
        paragraph_id=paragraph_node_id("PROG1", "A"), paragraph_name="A", decision_id=decision_id,
        detector_id="q0-return-code-decision", detector_version="1.0", detector_score=1.0,
        status=CandidateStatus.DETECTED_CANDIDATE, condition="CONDICION", outcome_code=outcome_code,
        line_start=line_start, source_file=SRC, source_package_hash=HASH,
    )
...
def _run(program: CanonicalProgram, *, decisions, data_item_tags, v1_candidates=None):
    ...
    return run_v2_shadow_detection(
        ctx, run_id="run1", source_package_hash=HASH,
...
def test_v2_only_when_v1_has_no_candidates() -> None:
    ...
    artifact = _run(
        program, decisions=[("A", 10, "CONDICION")],
...
def test_v1_only_when_v2_finds_no_evidence_for_that_decision() -> None:
    ...
    paragraph = CanonicalParagraph(
        name="A", source_text="A.", location_kind=LocationKind.UNKNOWN, statements=[if_stmt],
    )
    ...
    v1_candidates = CandidateArtifact(
        ...
        invariants_query_hash=HASH, q0_query_hash=HASH,
        candidates=[_v1_candidate(decision_id=decision_id, outcome_code="0005")],
    )
    artifact = _run(
        program, decisions=[("A", 10, "CONDICION")],
        data_item_tags={}, v1_candidates=v1_candidates,
    )
...
def test_run_v2_shadow_detection_is_byte_for_byte_deterministic() -> None:
    ...
    kwargs = dict(
        run_id="run1", source_package_hash=HASH,
        source_artifact_hashes={
            "artifacts/02-canonical": HASH,
            "artifacts/04-semantic-graph.json": HASH,
            "artifacts/06-candidates.json": HASH,
...
def _multi_decision_program() -> CanonicalProgram:
    ...
    mv3 = make_stmt(
        statement_id="P1::A::4::MOVE", target_data_items=["WS-COD-RETORNO"],
        variables_written=["WS-COD-RETORNO"], assigned_literal="0007",
        parent_statement_id="P1::A::3::IF", branch_kind="THEN",
    )
    paragraph = CanonicalParagraph(
        name="A", source_text="A.", location_kind=LocationKind.UNKNOWN,
        statements=[if1, mv1, if2, if3, mv3], variables_written=["WS-COD-RETORNO"],
    )
    return CanonicalProgram(
        program_name="PROG1", source_file=SRC, source_hash=HASH, source_package_hash=HASH,
        source_format=SourceFormat.FIXED, encoding="UTF-8", paragraphs=[paragraph],
...
def test_v2_v2_relation_never_increments_matched_v1_only_or_v2_only() -> None:
    ...
    paragraph = CanonicalParagraph(
        ...
        statements=[if_stmt, set_stmt], variables_written=["COD-CAMPO-INVALIDO"],
    )
    program = CanonicalProgram(
        program_name="PROG1", source_file=SRC, source_hash=HASH, source_package_hash=HASH,
        source_format=SourceFormat.FIXED, encoding="UTF-8",
        data_items=[
            CanonicalDataItem(
                name="WS-COD-RETORNO", qualified_name="WS-COD-RETORNO", level=1,
                location_kind=LocationKind.UNKNOWN,
            )
        ],
        condition_names=[condition], paragraphs=[paragraph],
    )
    artifact = _run(
        program, decisions=[("PARA", 10, "ERROR-DE-ENTRADA")],
        data_item_tags={"WS-COD-RETORNO": "return_code"},
    )
