"""Servicio de filesystem de propagacion limitada de constantes y copias
(Fase 4 de la ampliacion semantica, `feat/constant-copy-propagation`).
Unico punto que localiza un run, carga y valida `artifacts/02-canonical/`,
calcula su hash, calcula un `SemanticEffectsArtifact` EN MEMORIA (nunca lee
ni escribe `diagnostics/semantic-effects.json`: reutiliza el mismo
analizador puro de la Fase 2/3 como entrada intermedia, nunca como
artefacto persistido por este servicio), invoca
`semantic_propagation_analyzer.analyze_semantic_propagation` (analizador
puro) y persiste el resultado en `<run_dir>/diagnostics/
semantic-propagation.json`.

NO es un `PipelineStage`: se invoca exclusivamente bajo demanda (CLI
`semantic-propagation`), nunca desde `runner.py`/`run_ingestion`. Nunca
modifica ningun artefacto de entrada (`run.json`, `artifacts/02-canonical/`,
ni ningun otro de `artifacts/01-10`, ni `diagnostics/semantic-coverage.json`/
`diagnostics/semantic-effects.json` si ya existen); nunca escribe un
reporte parcial (si cualquier paso de carga/validacion/analisis falla, no
se crea/actualiza `diagnostics/semantic-propagation.json`).

Mismo patron que `semantic_effects_service.py` (Fase 2): replica
localmente la lectura/validacion minima de `run.json` que necesita, sin
importar `api/`."""
...
_REPORT_FILENAME = "semantic-propagation.json"
...
def _load_run_state(run_dir: Path) -> RunState:
    ...
    servicio necesita, sin importar `api/` (mismo patron que
    `semantic_effects_service.py`)."""
    run_json_path = run_dir / "run.json"
    if run_json_path.is_symlink() or not run_json_path.is_file():
        raise SemanticPropagationError(f"run {run_dir.name!r} no encontrado: run.json ausente")
    try:
        return RunState.model_validate_json(run_json_path.read_text(encoding="utf-8"))
    except ValueError as exc:
        raise SemanticPropagationError("run.json invalido") from exc
...
def _require_parsed(state: RunState) -> None:
    ...
    if execution is None or execution.status != StageStatus.SUCCEEDED:
        raise SemanticPropagationError(
            "el run no alcanzo PARSED (SUCCEEDED); no se puede calcular propagacion todavia"
        )


def _require_directory(path: Path, *, artifact_label: str) -> Path:
    if path.is_symlink() or not path.is_dir():
        raise SemanticPropagationError(f"{artifact_label} ausente o no es un directorio regular")
    return path
...
def _load_canonical_programs(canonical_dir: Path) -> list[CanonicalProgram]:
    ...
    for json_path in json_paths:
        ...
        try:
            ...
            raise SemanticPropagationError(f"{relative_label}: fallo de lectura") from exc
        try:
...
def compute_semantic_propagation_artifact(
    run_dir: Path, run_id: str
) -> SemanticPropagationArtifact:
    """Localiza `run_dir`, carga y valida `artifacts/02-canonical/`,
    calcula un `SemanticEffectsArtifact` en memoria (nunca lee/escribe
    `diagnostics/semantic-effects.json`), y devuelve el
    `SemanticPropagationArtifact` calculado (analizador puro: sin Neo4j,
    sin LLM, sin variables de entorno). Nunca escribe nada -- la
    persistencia es responsabilidad de
    `write_semantic_propagation_artifact`/el comando CLI."""
    if run_dir.is_symlink() or not run_dir.is_dir():
        raise SemanticPropagationError(f"run {run_dir.name!r} no encontrado")
...
def semantic_propagation_artifact_path(run_dir: Path) -> Path:
    return run_dir / _DIAGNOSTICS_DIR_NAME / _REPORT_FILENAME


def write_semantic_propagation_artifact(
    run_dir: Path, artifact: SemanticPropagationArtifact
) -> Path:
    """Persiste `artifact` de forma atomica en `diagnostics/
    semantic-propagation.json`. Nunca escribe un archivo parcial:
    `atomic_write_json` ya garantiza temporal-hermano + flush + fsync +
    replace; si el proceso se interrumpe antes del replace, el temporal
    se descarta y `diagnostics/semantic-propagation.json` no cambia (o no
    llega a existir)."""
    report_path = semantic_propagation_artifact_path(run_dir)
